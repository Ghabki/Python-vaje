...
ime = "Ljubljana"    #lahko spremenimo to ali pa z inputom
domet = 30

a = 0
d = 0
s = 0
n_kraj = "" #ni jalo napake ga pa je motlo da ni narejena spremeljivka prej

#samo da najde lokacijo kraja
...
for kraj, x, y in kraji:
    novi_x = d - x
    novi_y = s - y
    razdalja_od_lj = sqrt(novi_x**2 + novi_y**2)
    if razdalja_od_lj <= domet:
        if razdalja_od_lj > a:
            a = razdalja_od_lj
            n_kraj = kraj
print("Iz kraja" , ime , "lahko na najdaljsi razdalji zalijemo kraj" ,n_kraj , "na razdalji" , a)

#---------------------------------------------------------------------------------------------------------------------
print("Dodatna naloga:")
ime = "Ljubljana"  #podobno kot od zgoraj
ime_ena = "Bled"
domet = 30
...
#najdemo koordinate od obeh krajev in jih shranimo v nove spremenljivke
for kraj, x, y in kraji:
    if kraj == ime:                              #ce je ime isto
        v = x   #koordinate shrani v druge spremenljivke za nadaljno uporabo ker sem v kasnejem for zanki uporabil iste crke
        p = y
        print ("Kraj 2:",kraj,v,p)
    if kraj == ime_ena:
        o = x
        q = y
        print("Kraj 1:",kraj, o, q)

print("Iz kraja 1 in kraja 2 lahko zalivamo naslednje kraje:")
# In tu dopiši program
#2x gre v zanko
#prvic gre v prvi kraj in ga spet primerja y vsakim krajem ali ima razdaljo in
#ime kraja isto kot drugi kraj

for kraj_1, k, j in kraji:
    nova_x = v - k
    nova_y = p - j
    rezultat = sqrt(nova_x**2+nova_y**2)
    if rezultat < domet:
        for kraj_2, i, m in kraji:
            nova_x = o - i
            nova_y = q - m
            rezultat_a = sqrt(nova_x ** 2 + nova_y ** 2)
            if rezultat_a < 30:
                if kraj_1 == kraj_2:
                    print(kraj_1)